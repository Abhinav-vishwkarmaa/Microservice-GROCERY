...
const seller_table = `mtbl_seller`
const seller_otp_table = `mtbl_seller_otp`
export const register = async (req, res) => {
    const { name, email, password, mobile } = req.body;

    const existingUser = await query(
        `SELECT * FROM ${seller_table} WHERE email = ? OR mobile = ?`,
        [email, mobile]
    );
    ...
    if (existingUser.length > 0 && existingUser[0].is_verify === 0) {
        ...
        await query(
            `INSERT INTO ${seller_otp_table} (mobile, mobile_otp, email_otp, is_verify)
             VALUES (?, ?, ?, 0)
    ...
    await query(
        `INSERT INTO ${seller_table} (name, email, password, mobile, is_verify)
         VALUES (?, ?, ?, ?, 0)`,
        [name, email, hashedPassword, mobile]
    );

    await query(
        `INSERT INTO mtbl_seller_otp (mobile, mobile_otp, email_otp, is_verify)
         VALUES (?, ?, ?, 0)
         ON DUPLICATE KEY UPDATE 
            mobile_otp = VALUES(mobile_otp),
            email_otp = VALUES(email_otp),
...
export const verifyOtp = async(req,res) =>{
    ...
    const checkOtp = await query(`SELECT * FROM ${seller_otp_table} WHERE mobile = ? AND mobile_otp = ? AND email_otp = ? AND is_verify = ?`,[mobile,mobile_otp,email_otp,0])
    if(checkOtp.length > 0){
        if(checkOtp[0].is_verify ===1){
            res.status(400).json({message:"Otp already verified"})
        }else{
            await query(`UPDATE ${seller_otp_table} SET is_verify = ? WHERE mobile = ? AND mobile_otp = ? AND email_otp = ?`,[1,mobile,mobile_otp,email_otp])
            await query(`UPDATE ${seller_table} SET is_verify = ? WHERE mobile = ? AND email = ?`,[1,mobile,email])
...
export const login = async(req,res) =>{
    ...
    const checkUser = await query(`SELECT * FROM ${seller_table} WHERE email = ?`,[email])
